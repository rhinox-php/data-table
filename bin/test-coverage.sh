...
pushd $DIR/../
# @todo use paratest
vendor/bin/phpunit --order-by=random --coverage-xml .test-output/run_$DATE/coverage-xml --coverage-html .test-output/run_$DATE/coverage --log-junit .test-output/run_$DATE/report.xml -d memory_limit=-1 $@
